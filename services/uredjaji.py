...
def ucitaj_uredjaje() -> list:
    conn = get_db()
    return conn.execute("SELECT id, ime, cena, tip FROM uredjaji ORDER BY ime").fetchall()


def dodaj_uredjaj(ime: str, cena: float, tip: str) -> None:
    conn = get_db()
    conn.execute("INSERT INTO uredjaji (ime, cena, tip) VALUES (?, ?, ?)", (ime, cena, tip))
    conn.commit()
...
def seed_uredjaje_ako_prazno(podrazumijevani: list) -> None:
    ...
    if conn.execute("SELECT COUNT(*) FROM uredjaji").fetchone()[0] == 0:
        conn.executemany(
            "INSERT OR IGNORE INTO uredjaji (ime, cena, tip) VALUES (?, ?, ?)",
            podrazumijevani
        )
        conn.commit()
